package com.example.findmylocation_dipti_ict_04_05.viewmodel
...
class FireStoreViewModel_DIPTI_ICT_04_05: ViewModel() {
    ...
    fun getAllUsers(context: Context, callback: (List<User_DIPTI_ICT_04_05>) -> Unit) {
        usersCollection.get()
            .addOnSuccessListener {
                val userDIPTIICT0405List = mutableListOf<User_DIPTI_ICT_04_05>()
                for (document in it) {
                    val userId = document.id
                    val displayName = document.getString("displayName") ?: ""
                    val email = document.getString("email") ?: ""
                    val location = document.getString("location") ?: ""
                    userDIPTIICT0405List.add(User_DIPTI_ICT_04_05(userId, displayName, email, location))
                }
    ...
    fun getUser(context: Context, userId: String, callback: (User_DIPTI_ICT_04_05?) -> Unit) {
        usersCollection.document(userId).get()
            .addOnSuccessListener {
                val userDIPTIICT0405 = it.toObject(User_DIPTI_ICT_04_05::class.java)
